...
unsigned char* ImageLoader::Load(const char* filename, int& width, int& height, int& nrChannels, const bool bInvert)
{
...
unsigned int ImageLoader::LoadTextureGetID(const char* filename, const bool bInvert)
{
	// Load from file
	int image_width = 0;
	int image_height = 0;
	...
	if (data == NULL)
	{
		std::cout << "CImageLoader::LoadTextureGetID(): Unable to load " << FileSystem::getPath(filename).c_str() << std::endl;
		return 0;
	}
